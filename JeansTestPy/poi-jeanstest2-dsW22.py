...
class DSProfileW22(object):
    ...
    def thin_disk_density(self, R, z):
        ...
        # 0.5 * Sigma0thin / zdthin * exp(-fabs(Z) / zdthin - R / Rdthin);
...
if __name__ == "__main__":
    ...
    if True:
        ...
        zidx = 0
        for i in range(3):
                TR, Tz, dTR, dTz, NTR, DTR, MTR, GTR, NTz, DTz, MTz, GTz = _1d_data("gaia2", zlist[zidx], Rlist, model="W22", density_type="thin + thick profile")
                array_TR  .append(TR)
                array_Tz  .append(Tz)
        ...
        figR.savefig("TR-W22.pdf")
        figz.savefig("Tz-W22.pdf")
        # plt.show()
    ...
    if True:
        ...
        figR.savefig("TRtracers-W22.pdf")
        figz.savefig("Tztracers-W22.pdf")
    #     # plt.show()
    #     # plt.close()
    print("all set!")
    exit()