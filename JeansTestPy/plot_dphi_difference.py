...
for (ax1, z) in zip(axs, zs):
    ...
    h=0.1
    zipX = interpolator(Xi+h, Yi)
    zipY = interpolator(Xi, Yi+h)
    zimX = interpolator(Xi-h, Yi)
    zimY = interpolator(Xi, Yi-h)
    uiX = (zipX - zimX) / (2*h) / MpSStKPCpMYRMYR / 1000 * kpc / 1000
    uiY = (zipY - zimY) / (2*h) / MpSStKPCpMYRMYR / 1000 * kpc / 1000
    ...
    if cntr1 == 0:
        cntr1 = ax1.contourf(xi, yi, np.sqrt(uiX**2+uiY**2), levels=51, cmap="viridis", norm=norm)
    else:
        ax1.contourf(xi, yi, np.sqrt(uiX**2+uiY**2), levels=51, cmap="viridis", norm=norm)
...
for (ax1, z) in zip(axs, zs):
    ...
    ax1.quiver(xi,yi,uiX,uiY, color='white')



fig.colorbar(cntr1, ax = axs.ravel().tolist(), label=r"$\textbf{\textit{g}} - \textbf{\textit{g}}_{\rm N}~(\rm (km/s)^2/kpc)$", fraction=0.015, pad=0.03)
fig.text(0.5, 0.05, r"$R$ (kpc)", ha='center')
fig.text(0.05, 0.5, r"$z$ (kpc)", va='center', rotation='vertical')

plt.savefig("potential-acceleration-W21.pdf")
plt.show()